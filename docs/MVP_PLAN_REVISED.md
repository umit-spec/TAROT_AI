...
> section (the "Aşama 9" persistence stack) are **SUPERSEDED by ADR-009
> (PostgreSQL + Drizzle, Accepted)** and reaffirmed by the Product Owner's
> Phase 2 decision D2 (Drizzle authoritative; Prisma must not be introduced).
> Read `schema.prisma` below as a Drizzle schema (`src/db/schema.ts`).
> Persistence is designed in Phase 2 Sprint 4 against Neon PostgreSQL +
> Drizzle, not this section's stack.
